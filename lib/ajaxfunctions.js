...
exports.ask = function(req,res){
	...
	switch(question){
		case 'save':
			if(req.body.bilgi.bolum=="kategori"){
				...
				if(user instanceof Object){
					req.body.bilgi.ekleyen ={"name": user.email,"id": user._id};
					req.body.bilgi.ekleyen_id = user._id;
					functions.genel_kayit(req.body.bilgi,function(result){
	   					uutils.sendReturn(res,result);
	   				});
				}
				else{
					req.body.bilgi.ekleyen ={"name":JSON.parse(user).user_name,"id":JSON.parse(user)._id};
					req.body.bilgi.ekleyen_id = JSON.parse(user)._id;
					functions.genel_kayit(req.body.bilgi,function(result){
	   					uutils.sendReturn(res,result);
	   				});
		...
		case 'permission' :
			var permission = mdb.model('permission');
			var p = new permission();
			p.name = req.body.data.permissionName;
			p.p_id = req.body.data.permissionId;
			p.url  = req.body.data.permissionUrl;
			p.pageName = req.body.data.permissionPage ;
			p.save(function(err,sonuc){
		        if(err)
		          res.render('dashboard',{hata : err});
		        else
		          res.render('dashboard/role',{p : JSON.stringify(p)}); 
		     });  
      		
		break;

		case 'permissionList' :
			mdb.model("permission").find({},function(e,result){
				uutils.sendReturn(res,result);
			});
		break;
		...
		case 'sil':
			mdb.model(req.body.model).findOne({_id:req.body.id},function(e,kayit1){
				...
				else if(req.body.model=="permission"){
					var us=[];
					kayit1.remove(function(err,sonuc){
									if(err!=null)
										uutils.sendReturn(res,{status:err});
		...
		case 'user_role_update':
			mdb.model(req.body.model).findOne({_id:req.body.id},function(e,user){
				if((user.role.length>0) && (user.role[0]._id)){
		...
		case 'kbul':
		 ...
		 case 'sonOnline':
		     ...
		     var eleman =[];
		     mdb.model('user').find({},function(e,sonuclar){
		     	for(var i =0;i<sonuclar.length;i++){
		  			if(sonuclar[i].cevrimici[0].ip != null)
		  				eleman.push(sonuclar[i])	
	          	}
		...
		case 'datas' :
			var yorum =0;
			mdb.model("yorum").find({},function(e,yorum){
				for(var i=0;i<yorum.length;i++){
					if(yorum[i].ekleyen.id == req.body.userID)
						yorum += 1;
				}
				uutils.sendReturn(res,yorum);
			});
		break;

		case 'konu_data' :
			var konular = 0;
			mdb.model("konu").find({},function(e,konu){
				for(var i=0;i<konu.length;i++){
					if(konu[i].ekleyen.id == req.body.userID)
						konular = konular+1;
				}
				uutils.sendReturn(res,konular);
			});
		break;

		case 'roleName' :
			mdb.model('user').findOne({_id: req.body.id},function(e,usr){
				uutils.sendReturn(res,usr.role[0].role_name);  
		  });
		break;
		...
		case 'bildiriSayisi':
		  ...
		  mdb.model('bildiri').find({durum:"true"},function(e,veri){
		  	for(var i =0;i<veri.length;i++){
		  		if(req.body.id == veri[i].kime_bildiri.id)
		  			total.push(veri[i]);
		  	}
